...
import { type KnownTenant, TENANT_PATH_PREFIX } from '../../lib/tenant';
...
export default function TenantMeta({ knownTenants, tenantId }: TenantMetaProps) {
	const tenant = useMemo(
		() => knownTenants.find((t) => t.id === tenantId),
	...
	return (
		<span className="flex items-center gap-3 w-full">
			<img
				...
				alt={tenant.displayName || 'Default Tenant'}
				className="w-10 h-10 border border-lm-gray-400 dark:border-dm-gray-600 rounded-lg"
			></img>
			<span className="flex flex-col items-start gap-0.5">
				<span className="text-base">{tenant.displayName || 'Default Tenant'}</span>
				{tenant.userCount > 0 && (
					<span className="text-xs text-lm-gray-800 dark:text-dm-gray-200">
